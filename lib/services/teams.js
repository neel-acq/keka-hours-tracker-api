...
export async function sendTeamsMessage(userId, messageText) {
  ...
  // Check token expiry with 30-minute margin
  const now = new Date();
  const tokenExpiry = new Date(config.tokenExpiry);
  const margin = 30 * 60 * 1000; // 30 minutes

  if (tokenExpiry.getTime() - now.getTime() < margin) {
    return {
      success: false,
      error: 'Teams token expired or expiring soon. Please refresh by opening Teams.'
    };
  }
  ...
  if (!config.skypeToken || tokenExpiry < now) {
...
export async function computeSmartEodSuggestion(userId, scrapedAttendance, options = {}) {
  ...
  if (!inOutArray.length) {
    const now = new Date();
    return now.getHours() < 10 ? msg(0, 'Jai Dwarkadhish 🙏') : msg(3, 'Radhe Radhe 🙏');
  }
