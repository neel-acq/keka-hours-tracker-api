...
export function buildDayRecord(item) {
  ...
  if (item.originalTimeEntries && Array.isArray(item.originalTimeEntries)) {
    item.originalTimeEntries.forEach((timeEntry, index) => {
      ...
      if (!swipeType) return;
      const swipeDate = parseKekaTimestamp(timeEntry.timestamp);
      if (!swipeDate) return;
...
export function parseApiEntryForClient(item) {
  ...
  const inOutArray = (item.originalTimeEntries || []).map((te) => {
    ...
    const ts = parseKekaTimestamp(te.timestamp);
    return type && ts
      ? {
          type,
          time: formatTime12h(ts),
  ...
  return {
    ...
    shiftStart: item.shiftStartTime ? formatTime12h(parseKekaTimestamp(item.shiftStartTime)) : null,
    shiftEnd: item.shiftEndTime ? formatTime12h(parseKekaTimestamp(item.shiftEndTime)) : null,
